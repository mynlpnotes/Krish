...
* Work embedding (Feature Representation)
* Important parameters for this are vocabulary size and dimensions
* Dataset
  * Text                             Output
  * \<x11 x12 x13 x14 >         0
* word2vec is a type of word embedding and can also be used in embedding layer
* Lets say we have few words and there index, with a vocabulary of 10000
* For all the words we will select some feature representation, n which we will take every word and represent in some dimension(lets say 300)
* So the word boy will be converted into vector using the relation between boy and all the 300 features
* So using this we will be getting vectors for the word
*
